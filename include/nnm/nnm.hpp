...
template <typename Real>
constexpr Point3<Real> Point3<Real>::rotate_quaternion_at(const Point3& origin, const Quaternion<Real>& quaternion) const
{
    return transform_at(origin, Basis3<Real>::from_rotation_quaternion(quaternion));
}
...
{
    return from_vector((to_vector() - Vector3<Real> { origin.to_vector(), static_cast<Real>(0) }).transform(by)
